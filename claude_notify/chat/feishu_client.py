...
class FeishuClient:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    print(f"[feishu] 第 {attempt + 1} 次失败，{delay:.1f}s 后重试: {e}", flush=True)
                    await asyncio.sleep(delay)
                    delay *= 2
                else:
                    print(f"[feishu] 已达最大重试次数，放弃", flush=True)
    ...
    async def reply_card(self, message_id: str, content: str = "", loading: bool = True) -> str:
        ...
        return await self._retry_with_backoff(_reply)
    ...
    async def update_card(self, message_id: str, content: str):
        ...
        await self._retry_with_backoff(_update)
